...
- **⚠️ Column visibility ≠ calculation method**: The `hasAnyYield` check (which controls yield column visibility via `no-yield` CSS class) must NOT filter by borrow market status. The borrow market filter only determines which yield *calculation method* to use per-row (netflow vs snapshot), not whether yield data *exists*. If all supply assets are also borrow markets and `hasAnyYield` uses `!isBorrowMarket` in its netflow check, the entire column disappears even though netflow data exists and the per-row code correctly falls through to snapshot/dash display.
