...
The `Address` column is removed because the Portfolio page is scoped to one wallet entered above the table. The `Account` column keeps the existing truncated, monospace account number, hover explanation, and copy control. The full account number remains available through the existing copy behavior. Unknown fallback rows display an em dash.
