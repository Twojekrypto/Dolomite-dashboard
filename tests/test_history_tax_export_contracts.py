...
"""
    ...
    def test_history_is_positioned_as_transaction_history_first(self):
        ...
        self.assertNotIn("<h2>Transaction results</h2>", self.html)
        self.assertNotIn("<h2>Dolomite Transaction History</h2>", self.html)
        self.assertIn('<span class="beta-badge">BETA</span>', self.html)
        self.assertEqual(self.html.count("Wallet activity by date range"), 1)
        self.assertNotIn("Best-effort Dolomite history. Verify exports before accounting or tax filing.", self.html)
        ...
        self.assertIn('aria-label="Dolomite transaction history"', self.html)
    ...
    def test_history_header_is_unified_with_table_card(self):
        card_start = self.html.index('<section class="history-card')
        title_start = self.html.index("<h1>Dolomite Transaction History</h1>")
        toolbar_start = self.html.index('class="history-table-toolbar"')
        table_start = self.html.index('<div class="table-wrap">')
        self.assertLess(card_start, title_start)
        self.assertLess(title_start, toolbar_start)
        self.assertLess(toolbar_start, table_start)
        self.assertIn("history-primary-card", self.html)
        self.assertIn(".history-primary-card", self.css)
        self.assertIn(".card-title h1", self.css)
        self.assertNotIn(".history-hero", self.css)
        self.assertNotIn(".hero-topline", self.css)
...
"""
    ...
    def test_history_overview_consolidates_duplicate_summary_metrics(self):
        ...
        self.assertIn(".card-title h1{font-size:22px;line-height:1.12", self.css)
...
"""
    ...
    def test_history_report_is_consolidated_with_format_choices(self):
        ...
        self.assertIn("Wallet activity by date range: asset flows, gas evidence, reward candidates and export files.", self.html)
